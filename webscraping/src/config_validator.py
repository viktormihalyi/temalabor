...
def validate_spider_configuration(spider_config):
    ...
    warns_errors = _ErrorCounter()

    # must have a 'name'
    spider_name = _get_spider_name(spider_config)
    if not spider_name:
        logger.error('no name defined, exiting')
        return False

    enabled = _is_enabled(spider_config)
